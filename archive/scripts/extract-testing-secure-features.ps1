...
$deployments = kubectl --context kind-testing-cluster-secure `
    get deployments -n secure-ns -o json |
    ConvertFrom-Json

# Get NetworkPolicies from the secure cluster
$networkPolicies = kubectl --context kind-testing-cluster-secure `
    get networkpolicies -n secure-ns -o json |
    ConvertFrom-Json
...
$rows = foreach ($d in $deployments.items) {
    ...
    if ($appLabel) {
        ...
        $podName = kubectl --context kind-testing-cluster-secure `
            -n secure-ns `
            get pods `
            -l "app=$appLabel" `
        ...
        if ($podName) {
            ...
            $podInfo = kubectl --context kind-testing-cluster-secure `
                -n secure-ns `
                get pod $podName `
                -o json |
                ConvertFrom-Json
            ...
            foreach ($container in @($podInfo.spec.containers)) {
                ...
                $uidText = kubectl --context kind-testing-cluster-secure `
                    -n secure-ns `
                    exec $podName `
                    -c $container.name `
                    -- id -u `
...
$rows | Export-Csv `
    "reports\testing\testing-secure-features.csv" `
    -NoTypeInformation `
    -Encoding UTF8

Write-Host "Secure feature extraction complete."
Write-Host "Rows extracted: $($rows.Count)"
Write-Host "Output: reports\testing\testing-secure-features.csv"
